...
function emptyForm() {
  return { name: '', price: 0, sort_order: 0, is_active: true }
}
...
function openEdit(o) {
  ...
  form.value = { name: o.name, price: o.price, sort_order: o.sort_order, is_active: o.is_active }
  formError.value = ''
  showForm.value = true
}
...
async function onSave() {
  ...
  try {
    ...
    if (editingId.value) {
      await api.updateNominationFee(editingId.value, payload)
    } else {
      await api.createNominationFee(payload)
    }
...
async function onDelete(o) {
  ...
  error.value = ''
  try {
    await api.deleteNominationFee(o.id)
    await loadFees()
  } catch (e) {
    error.value = e.message
  }
  ...
  <LayoutOperator>
    ...
    <div class="card mb-4">
      <div class="card-header d-flex align-items-center justify-content-between">
        <span><i class="ti ti-star"></i> 指名料一覧</span>
        <button class="btn btn-primary btn-sm" @click="openCreate">
          <i class="ti ti-plus text-white"></i> 指名料追加
        </button>
      </div>
      <div class="card-body">
        ...
        <div v-else-if="!fees.length" class="text-muted text-center py-3">
          指名料が登録されていません
        </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editingId ? '指名料編集' : '指名料追加' }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input v-model="form.name" type="text" class="form-control" placeholder="例: 本指名" />
            </div>
            <div class="mb-3">
