...
function emptyForm() {
  return { name: '', discount_type: 'fixed', value: 0, sort_order: 0, is_active: true }
}
...
function openEdit(o) {
  ...
  form.value = { name: o.name, discount_type: o.discount_type, value: o.value, sort_order: o.sort_order, is_active: o.is_active }
  formError.value = ''
  showForm.value = true
}
...
function typeLabel(type) {
  return type === 'percent' ? '%割引' : '円引き'
}
</script>

<template>
  <LayoutOperator>
    <template #title>割引管理</template>
    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        ...
        <div v-else-if="!discounts.length" class="text-muted text-center py-3">
          割引が登録されていません
        </div>
        ...
        <table v-else class="table table-hover mb-0">
          ...
          <tbody>
            <tr v-for="o in discounts" :key="o.id">
              <td>{{ o.name }}</td>
              <td>{{ typeLabel(o.discount_type) }}</td>
              <td>{{ o.discount_type === 'percent' ? o.value + '%' : o.value.toLocaleString() + '円' }}</td>
              <td>{{ o.sort_order }}</td>
              <td>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editingId ? '割引編集' : '割引追加' }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <input v-model="form.name" type="text" class="form-control" placeholder="例: 会員割引" />
            </div>
            <div class="mb-3">
              <label class="form-label">種別 <span class="text-danger">*</span></label>
              <select v-model="form.discount_type" class="form-select">
                <option value="fixed">円引き（固定額）</option>
                <option value="percent">%割引</option>
              </select>
            </div>
            <div class="mb-3">
              <label class="form-label">
                値 <span class="text-danger">*</span>
                <small class="text-muted">{{ form.discount_type === 'percent' ? '（%）' : '（円）' }}</small>
              </label>
